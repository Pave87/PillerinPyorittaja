...
namespace MauiBlazorHybrid
{
    ...
    public class MainActivity : MauiAppCompatActivity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            {
                _loggerService.Log("OnCreate called");
        ...
        protected override void OnNewIntent(Intent intent)
        {
            using (CallContext.BeginCall())
            {
                _loggerService.Log("OnNewIntent called");
        ...
        private void ProcessIntent(Android.Content.Intent intent)
        {
            using (CallContext.BeginCall())
            {
                _loggerService.Log("ProcessIntent called");
                if (intent != null && intent.HasExtra("action"))
                {
                    string action = intent.GetStringExtra("action");
                    _loggerService.Log("Intent action: {action}");
                    ...
                    if (action == "edit_reminder")
                    {
                        ...
                        if (productId > 0 && dosageId > 0)
                        {
                            ...
                            _loggerService.Log("Processed intent with productId={productId}, dosageId={dosageId}, remindLater={remindLater}");
                        }
                    }
                    else if (action == "take_now_action")
                    {
                        ...
                        if (productService != null)
                        {
                            Task.Run(async () =>
                            {
                                ...
                                _loggerService.Log("Processed take_now_action with productId={productId}, dosageId={dosageId}, amount={amount}, success={success}");
                            });
                        }
                    }
                                ...
                                try
                                {
                                    ...
                                    if (product != null)
                                    {
                                        ...
                                        if (dosage != null)
                                        {
                                            ...
                                            _loggerService.Log("Scheduled remind_15_action for productId={productId}, dosageId={dosageId} at {remindTime}");
                                        }
                                        else
                                        {
                                            _loggerService.Log("Dosage with ID {dosageId} not found for productId={productId}");
                                        }
                                    }
                                    else
                                    {
                                        _loggerService.Log("Product with ID {productId} not found");
                                    }
                                }
                                catch (Exception ex)
                                {
                                    _loggerService.Log("Error processing remind_15_action: {ex.Message}");
                                }
                            });
                        }
                    }
